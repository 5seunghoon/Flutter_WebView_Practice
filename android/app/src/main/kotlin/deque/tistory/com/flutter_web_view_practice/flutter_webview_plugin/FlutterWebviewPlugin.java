...
public class FlutterWebviewPlugin implements MethodCallHandler, PluginRegistry.ActivityResultListener {
    ...
    public static void registerNormal(FlutterActivity flutterActivity) {
        channel = new MethodChannel(flutterActivity.getFlutterView(), CHANNEL_NAME);
        Log.d("LOGGER_TAG", "register normal webview plugin");
        final FlutterWebviewPlugin instance = new FlutterWebviewPlugin(flutterActivity, flutterActivity.getApplicationContext());
        channel.setMethodCallHandler(instance);
    }
    ...
    private void captureWebview(MethodCall call, MethodChannel.Result result) {
        int tabId = 0;
        try {
            tabId = call.argument("tabId");
        } catch (NullPointerException e) {
            e.printStackTrace();
        }
        webViewManager.capture(tabId);
        result.success(null);
    }
